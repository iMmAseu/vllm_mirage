...
class cmake_build_ext(BuildExtension):
...
if _is_cuda():
    ...
    # Mirage integration - fixed multiple definition errors
    mirage_src_dir = ROOT_DIR / "csrc" / "mirage"
    if mirage_src_dir.exists():
        # Use relative paths for editable install compatibility
        mirage_sources = sorted(str(p.relative_to(ROOT_DIR)) for p in mirage_src_dir.rglob("*.cu"))
        if mirage_sources:
            mirage_root = (ROOT_DIR.parent / "mirage").resolve()
            mirage_include = mirage_root / "include"
            mirage_cutlass_include = mirage_root / "deps" / "cutlass" / "include"
            mirage_json_include = mirage_root / "deps" / "json" / "include"

            mirage_runtime_include = mirage_include / "mirage" / "transpiler" / "runtime"
            include_dirs = [str(mirage_include)]
            if mirage_runtime_include.exists():
                include_dirs.append(str(mirage_runtime_include))
            if mirage_cutlass_include.exists():
                include_dirs.append(str(mirage_cutlass_include))
            if mirage_json_include.exists():
                include_dirs.append(str(mirage_json_include))

            extra_compile_args = {
                "cxx": ["-O3", "-std=c++17", "-DMIRAGE_BACKEND_USE_CUDA"],
            ...
            ext_modules.append(
                CUDAExtension(
                    name="vllm._mirage_ops",
                    sources=mirage_sources,  # Already strings with relative paths
