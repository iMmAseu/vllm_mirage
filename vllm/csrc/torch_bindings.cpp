...
// Helper to lazily look up Mirage operator handles.
template <typename Signature>
c10::TypedOperatorHandle<Signature> get_mirage_op(const char* schema) {
...
using MirageSignature =
    void(int64_t, const at::TensorList&, const at::TensorList&);

void mirage_rope(int64_t batch, const at::TensorList& inputs,
                 const at::TensorList& outputs) {
  static auto op =
      get_mirage_op<MirageSignature>("mirage_ops::rope_run");
  op.call(batch, inputs, outputs);
}

void mirage_attention(int64_t batch, const at::TensorList& inputs,
                      const at::TensorList& outputs) {
  static auto op =
      get_mirage_op<MirageSignature>("mirage_ops::attention_run");
  op.call(batch, inputs, outputs);
}

void mirage_qkv_rmsnorm(int64_t batch, const at::TensorList& inputs,
                        const at::TensorList& outputs) {
  static auto op =
      get_mirage_op<MirageSignature>("mirage_ops::qkv_rmsnorm_run");
  op.call(batch, inputs, outputs);
}

void mirage_rmsnorm_only(int64_t batch, const at::TensorList& inputs,
                         const at::TensorList& outputs) {
  static auto op =
      get_mirage_op<MirageSignature>("mirage_ops::rmsnorm_only_run");
  op.call(batch, inputs, outputs);
}

void mirage_ffn(int64_t batch, const at::TensorList& inputs,
                const at::TensorList& outputs) {
  static auto op =
      get_mirage_op<MirageSignature>("mirage_ops::ffn_run");
  op.call(batch, inputs, outputs);
}
...
TORCH_LIBRARY_EXPAND(TORCH_EXTENSION_NAME, ops) {
  ...
  // Mirage operators (wrap implementations from vllm._mirage_ops)
  ops.def("mirage_rope(int batch, Tensor[] inputs, Tensor[] outputs) -> ()");
  ops.impl("mirage_rope", torch::kCUDA, &mirage_rope);

  ops.def(
      "mirage_attention(int batch, Tensor[] inputs, Tensor[] outputs) -> ()");
  ops.impl("mirage_attention", torch::kCUDA, &mirage_attention);

  ops.def(
      "mirage_qkv_rmsnorm(int batch, Tensor[] inputs, Tensor[] outputs) -> ()");
  ops.impl("mirage_qkv_rmsnorm", torch::kCUDA, &mirage_qkv_rmsnorm);

  ops.def(
      "mirage_rmsnorm_only(int batch, Tensor[] inputs, Tensor[] outputs) -> ()");
  ops.impl("mirage_rmsnorm_only", torch::kCUDA, &mirage_rmsnorm_only);

  ops.def("mirage_ffn(int batch, Tensor[] inputs, Tensor[] outputs) -> ()");
  ops.impl("mirage_ffn", torch::kCUDA, &mirage_ffn);
