...
# Mirage operators
# These operators provide fused GPU kernels compiled by Mirage
supports_mirage_ops = False
try:
    import vllm._mirage_ops  # noqa: F401
    supports_mirage_ops = True
except ImportError:
    logger.info("Mirage operators not available")


def mirage_rope(batch: int, inputs: list, outputs: list) -> None:
    """
    Mirage RoPE (Rotary Position Embedding) operation.

    Args:
        batch: Batch size (must be 1, 2, 4, or 8)
        inputs: List of 10 input tensors
        outputs: List of 4 output tensors
    """
    if not supports_mirage_ops:
        raise RuntimeError(
            "Mirage operators not available. Please compile vLLM with Mirage support."
        )
    torch.ops._C.mirage_rope(batch, inputs, outputs)


def mirage_attention(batch: int, inputs: list, outputs: list) -> None:
    """
    Mirage Attention operation.

    Args:
        batch: Batch size (must be 1, 2, 4, or 8)
        inputs: List of 42 input tensors
        outputs: List of 1 output tensor
    """
    if not supports_mirage_ops:
        raise RuntimeError(
            "Mirage operators not available. Please compile vLLM with Mirage support."
        )
    torch.ops._C.mirage_attention(batch, inputs, outputs)


def mirage_qkv_rmsnorm(batch: int, inputs: list, outputs: list) -> None:
    """
    Mirage QKV + RMSNorm fused operation.

    Args:
        batch: Batch size (must be 1, 2, 4, or 8)
        inputs: List of 16 input tensors
        outputs: List of 1 output tensor
    """
    if not supports_mirage_ops:
        raise RuntimeError(
    ...
    torch.ops._C.mirage_qkv_rmsnorm(batch, inputs, outputs)


def mirage_rmsnorm_only(batch: int, inputs: list, outputs: list) -> None:
    """
    Mirage RMSNorm-only operation.

    Args:
        batch: Batch size (must be 1, 2, 4, or 8)
        inputs: List of 2 input tensors (input, weight)
        outputs: List of 1 output tensor
    """
    if not supports_mirage_ops:
        raise RuntimeError(
    ...
    torch.ops._C.mirage_rmsnorm_only(batch, inputs, outputs)


def mirage_ffn(batch: int, inputs: list, outputs: list) -> None:
    """
    Mirage FFN (Feed-Forward Network) operation.

    Args:
        batch: Batch size (must be 1, 2, 4, or 8)
        inputs: List of 7 input tensors
        outputs: List of 1 output tensor
    """
    if not supports_mirage_ops:
        raise RuntimeError(
    ...
    torch.ops._C.mirage_ffn(batch, inputs, outputs)
