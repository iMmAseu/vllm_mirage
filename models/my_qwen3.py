...
class Qwen3Model(Qwen3PreTrainedModel):
    def __init__(self, config: Qwen3Config, world_size: int = 1, max_num_pages: int = 16, page_size: int = 4096):
...
class Qwen3ForCausalLM(Qwen3PreTrainedModel, GenerationMixin):
    ...
    def __init__(self, config, world_size=1, max_num_pages=16, page_size=4096):
        super().__init__(config)
    ...
    @classmethod
    def from_pretrained(cls, pretrained_model_name_or_path, *model_args, world_size=1, max_num_pages=16, page_size=4096, **kwargs):
        """
        Load pretrained model with custom KV-cache parameters.

        Args:
            pretrained_model_name_or_path: Model identifier from huggingface.co/models or local path
            world_size: Number of GPUs for tensor parallelism (default: 1)
            max_num_pages: Maximum number of KV-cache pages (default: 16)
            page_size: Size of each KV-cache page (default: 4096)
        ...
        model = cls(config, world_size=world_size, max_num_pages=max_num_pages, page_size=page_size)

        # Load pretrained weights
        # Use HuggingFace's standard weight loading
