...
__global__ void __launch_bounds__(128) custom_kernel_1(bfloat16_t* __restrict__ dtensor10000008_ptr, bfloat16_t const* __restrict__ dtensor10000006_ptr, bfloat16_t const* __restrict__ dtensor10000007_ptr) {
  ...
  bfloat16_t *stensor20000051_ptr = (bfloat16_t*)(buf + 304);
  bfloat16_t *stensor20000049_ptr = (bfloat16_t*)(buf + 288);
  bfloat16_t *stensor20000050_ptr = (bfloat16_t*)(buf + 160);
  bfloat16_t *stensor30000043_ptr = (bfloat16_t*)(buf + 2272);
  bfloat16_t *stensor20000043_ptr = (bfloat16_t*)(buf + 224);
  bfloat16_t *stensor30000042_ptr = (bfloat16_t*)(buf + 192);
  bfloat16_t *stensor20000042_ptr = (bfloat16_t*)(buf + 160);
  ...
  *((uint128_t*)buf) = 0ul;
  ...
  using DTensor10000006TileLayout = Layout<Shape<Int<16>, Int<1>>, Stride<Int<1>, Int<1024>>>;
  using STensor20000042InputAtom = tb::InputChunkedAsyncCopy<bfloat16_t, Layout<Shape<Int<16>, Int<1>>, Stride<Int<1>, Int<16>>>, DTensor10000006TileLayout, NUM_THREADS>;
  ...
  using DTensor10000007TileLayout = Layout<Shape<Int<64>, Int<16>>, Stride<Int<1>, Int<3072>>>;
  ...
  bfloat16_t *dtensor10000008_tile_ptr = dtensor10000008_ptr  + blockIdx.x*64*1;
  using DTensor10000008TileLayout = Layout<Shape<Int<64>, Int<1>>, Stride<Int<1>, Int<1>>>;
  using STensor20000051OutputAtom = tb::OutputChunkedSyncCopy<bfloat16_t, DTensor10000008TileLayout, Layout<Shape<Int<64>, Int<1>>, Stride<Int<1>, Int<1>>>, NUM_THREADS>;
  
  tb::ClearAccumlatorKernel<bfloat16_t, 16, NUM_THREADS>::run(stensor20000047_ptr, thread_idx);
  
  
  using Matmul20000050LayoutA = Layout<Shape<Int<16>, Int<1>>, Stride<Int<1>, Int<16>>>;
  using Matmul20000050LayoutB = decltype(composition(Swizzle<3, 3, 3>{}, Layout<Shape<Int<64>, Int<16>>, Stride<Int<1>, Int<64>>>{}));
  using Matmul20000050LayoutC = Layout<Shape<Int<64>, Int<1>>, Stride<Int<1>, Int<64>>>;
  using Matmul20000050LayoutAAligned = Layout<Shape<Int<16>, Int<16>>, Stride<Int<1>, Int<16>>>;
  ...
  for (int for_idx = 0; for_idx < 64; for_idx++) {
    {
      ...
      if (for_idx+1 != 64) {
        STensor20000043InputAtom::run(stensor20000043_ptr, dtensor10000007_tile_ptr + 49152*(for_idx+1), thread_idx);
    ...
    }
    {
      ...
      using InLayout = Layout<Shape<Int<1>, Int<16>>, Stride<Int<16>, Int<1>>>;
      using OutLayout = Layout<Shape<Int<1>, Int<16>>, Stride<Int<1>, Int<1>>>;
  ...
  __syncthreads();
  {
    ...
    using InLayout = Layout<Shape<Int<1>, Int<16>>, Stride<Int<1>, Int<1>>>;
...
static void _execute_mugraph(std::vector<void const *> input_tensors, std::vector<void*> output_tensors, void* buf, cudaStream_t stream, void * profiler_buffer){
  ...
  }
  {
    ...
    dim3 grid_dim(48, 1, 1);
    dim3 block_dim(128, 1, 1);
    size_t smem_size = 4320;
    
    // define tmas
    cudaFuncSetAttribute(custom_kernel_1, cudaFuncAttributeMaxDynamicSharedMemorySize, 4320);
